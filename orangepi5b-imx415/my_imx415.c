...
static ssize_t imx415_fps_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	fps = imx415->cur_fps.denominator / imx415->cur_fps.numerator;
	return sprintf(buf, "current fps: %d\n"
			    "vts_def: %d  cur_vts: %d  height: %d\n"
			    "usage: echo <fps> > fps\n",
			    fps,
			    imx415->cur_mode->vts_def,
			    imx415->cur_vts,
			    imx415->cur_mode->height);
}

static ssize_t imx415_fps_store(struct device *dev,
				 ...
				 const char *buf, size_t count)
{
	...
	u32 target_fps, vts, vblank;
	u32 hts;
	u64 pixel_clock;
	int ret;

	ret = kstrtouint(buf, 10, &target_fps);
	if (ret || target_fps == 0 || target_fps > 120) {
		dev_err(dev, "invalid fps: %s (valid: 1~120)\n", buf);
		return -EINVAL;
	}

	mutex_lock(&imx415->mutex);

	hts = imx415->cur_mode->hts_def;
	pixel_clock = (u64)hts * imx415->cur_mode->vts_def *
		      imx415->cur_mode->max_fps.denominator /
		      imx415->cur_mode->max_fps.numerator;
	vts = (u32)(pixel_clock / hts / target_fps);

	if (vts < imx415->cur_mode->height + 46)
		vts = imx415->cur_mode->height + 46;
	if (vts > IMX415_VTS_MAX)
		vts = IMX415_VTS_MAX;

	vblank = vts - imx415->cur_mode->height;
	imx415->cur_vts = vts;

	ret = imx415_write_reg(client, IMX415_VTS_REG_L,
			       IMX415_REG_VALUE_08BIT,
			       IMX415_FETCH_VTS_L(vts));
	ret |= imx415_write_reg(client, IMX415_VTS_REG_M,
				IMX415_REG_VALUE_08BIT,
				IMX415_FETCH_VTS_M(vts));
	ret |= imx415_write_reg(client, IMX415_VTS_REG_H,
				IMX415_REG_VALUE_08BIT,
				IMX415_FETCH_VTS_H(vts));

	__v4l2_ctrl_s_ctrl(imx415->vblank, vblank);
	imx415_modify_fps_info(imx415);

	mutex_unlock(&imx415->mutex);

	dev_info(dev, "set fps=%d, vts=%d, vblank=%d, ret=%d\n",
		 target_fps, vts, vblank, ret);

	return ret ? ret : count;
}

static DEVICE_ATTR(fps, 0644, imx415_fps_show, imx415_fps_store);

/* ========== sysfs: 查看当前模式信息 ========== */

static ssize_t imx415_mode_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct v4l2_subdev *sd = dev_get_drvdata(dev);
	struct imx415 *imx415 = to_imx415(sd);
	...
	return sprintf(buf,
		"resolution : %dx%d\n"
		"hdr_mode   : %d (0=Linear 1=HDR_X2 2=HDR_X3)\n"
		"bpp        : %d\n"
		"max_fps    : %d\n"
		"cur_fps    : %d\n"
		"vts_def    : %d\n"
		...
		imx415->cur_fps.denominator / imx415->cur_fps.numerator,
		mode->vts_def,
		imx415->cur_vts,
		mode->hts_def);
}

static DEVICE_ATTR(mode_info, 0444, imx415_mode_show, NULL);
...
static __maybe_unused const struct regval imx415_linear_10bit_1944x1097_1782M_regs[] = {
	/* 1. 全局与模式设置 */
    {0x301C, 0x00}, // WINMODE: All pixel mode
    {0x3031, 0x00}, // ADBIT: 10 bit
    {0x3033, 0x04}, // SYS_MODE: 4h (60fps 模式特有)
    ...
    /* 2. 帧率与曝光边界 (HMAX / VMAX) */
    {0x3024, 0xCA}, // VMAX LSB (8CAh)
    {0x3025, 0x08}, // VMAX MSB
	{0x3026, 0x00}, // VMAX MSB
    {0x3028, 0x26}, // HMAX LSB (226h)
    {0x3029, 0x02}, // HMAX MSB

	{0x30D9, 0x02}, // DIG_CLP_VSTAET
	{0x30DA, 0x01}, // DIG_VLP_VNUM

    /* 3. INCK / PLL 设置 (核心补齐部分) */
    {0x3008, 0x7F}, // BCWAIT_TIME LSB
    {0x3009, 0x00}, // BCWAIT_TIME MSB
    {0x300A, 0x5B}, // CPWAIT_TIME LSB
    {0x300B, 0x00}, // CPWAIT_TIME MSB
    {0x3115, 0x00}, // INCKSEL1
    {0x3116, 0x24}, // INCKSEL2
    {0x3118, 0xC0}, // INCKSEL3 LSB
    {0x3119, 0x00}, // INCKSEL3 MSB
    {0x311A, 0xE0}, // INCKSEL4 LSB
    {0x311B, 0x00}, // INCKSEL4 MSB
    {0x311E, 0x24}, // INCKSEL5
    {0x4004, 0x48}, // TXCLKESC_FREQ LSB
    {0x4005, 0x09}, // TXCLKESC_FREQ MSB
    {0x400C, 0x01}, // INCKSEL6
    {0x4074, 0x00}, // INCKSEL7

    /* 4. MIPI DPHY 全局时序控制 (匹配 1782Mbps) */
    {0x4018, 0xB7}, // TCLKPOST LSB
    {0x4019, 0x00}, // TCLKPOST MSB
    {0x401A, 0x67}, // TCLKPREPARE LSB
    {0x401B, 0x00}, // TCLKPREPARE MSB
    {0x401C, 0x6F}, // TCLKTRAIL LSB
    {0x401D, 0x00}, // TCLKTRAIL MSB
    {0x401E, 0xDF}, // TCLKZERO LSB
    {0x401F, 0x01}, // TCLKZERO MSB
    {0x4020, 0x6F}, // THSPREPARE LSB
    {0x4021, 0x00}, // THSPREPARE MSB
    {0x4022, 0xCF}, // THSZERO LSB
    {0x4023, 0x00}, // THSZERO MSB
	{0x4024, 0x6F}, // TCLKPREPARE MSB
    {0x4025, 0x00}, // TCLKTRAIL LSB
    {0x4026, 0xB7}, // TCLKTRAIL MSB
    {0x4027, 0x00}, // TCLKZERO LSB
    {0x4028, 0x5F}, // TCLKZERO MSB
    {0x4029, 0x00}, // THSPREPARE LSB
...
static __maybe_unused const struct regval imx415_linear_10bit_3864x2192_2376M_regs[] = {
    /* 时钟与等待时间配置 (37.125MHz) */
    {0x3008, 0x7F},   // BCWAIT_TIME 低字节 (07Fh)
    {0x3009, 0x00},   // BCWAIT_TIME 高字节
    {0x300A, 0x5B},   // CPWAIT_TIME 低字节 (05Bh)
    {0x300B, 0x00},   // CPWAIT_TIME 高字节
    {0x3020, 0x00},
    {0x3021, 0x00},
    {0x3022, 0x00},
    {0x3024, 0xCA},
    {0x3025, 0x08},
    {0x3028, 0x6E},   // HMAX 低字节 (16Eh)
    {0x3029, 0x01},   // HMAX 高字节
    {0x302C, 0x00},
    {0x302D, 0x00},
    {0x3033, 0x00},   // SYS_MODE = 0h (90fps)
    {0x3050, 0x08},
    {0x3051, 0x00},
    {0x3054, 0x19},
    {0x3058, 0x3E},
    ...
    {0x30CF, 0x00},
    {0x3116, 0x24},   // INCKSEL2
    {0x3118, 0x00},   // INCKSEL3 低字节 (100h)
    {0x3119, 0x01},   // INCKSEL3 高字节
    {0x311A, 0xE0},   // INCKSEL4 低字节 (0E0h)  ← 补齐
    {0x311B, 0x00},   // INCKSEL4 高字节         ← 补齐
    {0x311E, 0x24},   // INCKSEL5
    {0x3260, 0x01},
    {0x4004, 0x48},   // TXCLKESC_FREQ 低字节 (0948h)
    {0x4005, 0x09},   // TXCLKESC_FREQ 高字节
    {0x400C, 0x01},   // INCKSEL6

    /* 全局时序 Global timing (2376Mbps) */
    {0x4018, 0xE7},
    {0x401A, 0x8F},
    {0x401C, 0x8F},
    {0x401E, 0x7F},
...
static const struct imx415_mode supported_modes[] = {
	 ...
	 */
	{
		...
		.exp_def = 0x08ca - 0x08, //CA大写改小写
		.hts_def = 0x0226 * IMX415_4LANES * 4,//*2 改*4
		.vts_def = 0x08ca, //CA大写改小写
		.global_reg_list = NULL, //改成NULL
		.reg_list = imx415_linear_10bit_3864x2192_1782M_regs, // <--- 使用刚刚添加的寄存器组
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 3,  // <--- 提高带宽，解决之前提到的溢出问题
		.bpp = 10,
	...
	#if 0
	{
		...
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 4,   // MIPI_FREQ_1188M → 2376Mbps/lane
		.bpp = 10,
		.vc[PAD0] = V4L2_MBUS_CSI2_CHANNEL_0,
		.xvclk = IMX415_XVCLK_FREQ_37M,
	},
	{
		...
		.exp_def = 0x08ca - 0x08,
		.hts_def = 0x030E * IMX415_4LANES * 2,
		.vts_def = 0x08ca,
		.global_reg_list = imx415_global_10bit_3864x2192_regs,
	...
	},
	{
		...
		.exp_def = 0x08ca - 0x08,   // 参考同速率全像素60fps条目的算法：vts_def - 固定偏移
		.hts_def = 0x0226 * IMX415_4LANES * 2,   // 注意：现有条目hts_def都是 HMAX寄存器值 * lane数 * 2，不是直接抄HMAX原始值
		.vts_def = 0x08ca,           // 对应 VMAX 8CAh，这个可以直接用寄存器原始值
		.global_reg_list = imx415_global_10bit_3864x2192_regs,
		.reg_list = imx415_linear_10bit_1944x1097_1782M_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 3,          // 对应1782Mbps，不是0
		.bpp = 10,
		.vc[PAD0] = V4L2_MBUS_CSI2_CHANNEL_0,
		.xvclk = IMX415_XVCLK_FREQ_37M,
	},
			...
			const struct i2c_device_id *id)
{
	...
	/* 注册 sysfs */
	ret = sysfs_create_group(&dev->kobj, &imx415_attr_group);
	if (ret)
...
static int imx415_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &imx415_attr_group);
...
static struct i2c_driver imx415_i2c_driver = {
    .driver = {
        .name = IMX415_NAME,  // <--- 强行改名，避开内核自带的同名驱动
        .pm = &imx415_pm_ops,
